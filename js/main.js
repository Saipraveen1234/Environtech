(function ($) {
    ...
    function updateCalculatorResults(systemSize) {
        ...
        const annualSavings = Math.round(systemSize * SAVINGS_PER_KW * locationFactor);
    ...
    */
    ...
    function handleQuoteFormSubmission() {
        $('#quoteForm').on('submit', function(e) {
            ...
            // Show loading state
            $('.btn-submit').html('<i class="fas fa-spinner fa-spin"></i> Submitting...').prop('disabled', true);
    ...
    function sendQuoteEmail(formData) {
        ...
        if (EMAIL_CONFIG.serviceID === 'YOUR_SERVICE_ID') {
            ...
            setTimeout(() => {
                ...
                // Reset button
                $('.btn-submit').html('<i class="fas fa-paper-plane"></i> Get Free Quote').prop('disabled', false);
